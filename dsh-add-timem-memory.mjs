 ...
 *   6. 连接验证（--verify）、DSH 重启引导
 *   7. 维护：--uninstall 卸载、--check-update / --update 升级
 *
...
const ENTRY_ID = 'mcp-timem';
const DEFAULTS = {
  serverName: 'timem',
  url: 'https://api.timem.cloud/mcp',
  githubRepo: 'aimaoge/timem-dsh-mcp',
};
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (name) {
      ...
      case '--user-id': opts.userId = val; break;
...
安装:
  （无参数）          自动查找 cordis.patch.yml + 交互输入 key/user-id + 验证 + 装 skill + 询问重启
  --file <path>       指定 cordis.patch.yml（跳过自动查找）
  --key <key>         API Key（优先级: --key > 环境变量 TiMEM_API_KEY > 交互输入）
  --user-id <id>      用户 ID（优先级: --user-id > TiMEM_USER_ID > 交互输入，可留空）
...
function buildBlock({ serverName, url, key, userId, eol }) {
  const L = [
    '',
    '# ---- TiMEM MCP 桥接（DSH 版 mcpServers 示例）----',
    '# 等价于其他平台 JSON: {"mcpServers":{"TiMEM-MCP":{"url":"' + url + '","headers":{"X-API-Key":"...","X-TiMEM-User-Id":"..."}}}}',
    `- id: ${ENTRY_ID}`,
  ...
  if (userId) L.push(`      X-TiMEM-User-Id: ${yamlScalar(userId)}`);
  return L.join(eol);
}
...
async function main() {
  ...
  // 2b) 获取用户 ID（优先级: --user-id > 环境变量 TiMEM_USER_ID > 交互输入）
  let userId = opts.userId;
  if (userId === null) userId = process.env.TiMEM_USER_ID || null;
  if (userId === null) {
    userId = await askSecret('请输入 TiMEM 用户 ID（X-TiMEM-User-Id，可留空）: ');
    if (userId === null) {
      console.error('[错误] 已取消。');
      process.exit(1);
    }
  }

  // 2c) 仅验证模式：不找配置、不写文件，只测端点/认证/工具
  if (opts.verify) {
    const ok = await verifyConnection(url, key, userId);
    process.exit(ok ? 0 : 1);
  }
  ...
  const raw = fs.readFileSync(file, 'utf8');
  const eol = raw.includes('\r\n') ? '\r\n' : '\n';
  const block = buildBlock({ serverName, url, key, userId, eol });
  ...
  console.log('        （如 mcp__timem__search_memories、mcp__timem__create_memory）\n');
  const headers = { 'X-API-Key': key };
  if (userId) headers['X-TiMEM-User-Id'] = userId;
