...
- 环境变量 `TiMEM_API_KEY` / `TiMEM_USER_ID`（优先级：命令行 > 环境变量 > 交互输入），避免密钥明文进命令历史
...
- 仓库更名 `timem-dsh-mcp` → `timem-dsh-memory`（包名 / bin / 脚本 / remote 同步）
- `timemspace-dsh-mcp` → `timemspace-dsh-memory`
