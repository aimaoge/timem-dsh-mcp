...
一键向 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 集成 **TiMEM 主平台记忆**：MCP 桥接 + 配套 skill + 连接验证 + 重启引导。
...
本工具把配置以 `@deepseek-ai/dsh-mcp-client` 插件实例写入 DSH profile 的 `cordis.yml`，并**自动安装配套 skill**（`timem-general-memory` 等）。重启 DSH host 后：工具以 `mcp__timem__*` 命名注册，skill 提供"何时检索、何时写入"的钩子——**MCP 工具 + skill 才是完整记忆链路**。
...
```bash
# 自动查找 cordis.yml + 交互式输入 API Key 与用户 ID（粘贴不回显，用户 ID 可留空）
npx --yes github:aimaoge/timem-dsh-memory

# 指定配置文件 / 免交互（脚本、CI 用）
npx --yes github:aimaoge/timem-dsh-memory --file /path/to/cordis.yml --key sk-xxxx --user-id usr_xxx -y

# 只预览不落盘
npx --yes github:aimaoge/timem-dsh-memory --dry-run --key sk-xxxx

# 装完 TiMEM 后先验证 key/URL 是否可用（不修改配置）
npx --yes github:aimaoge/timem-dsh-memory --verify --key sk-xxxx --user-id usr_xxx
```
...
> - PowerShell：`$env:npm_config_strict_ssl='false'; npx --yes github:aimaoge/timem-dsh-memory`
> - bash/zsh：`npm_config_strict_ssl=false npx --yes github:aimaoge/timem-dsh-memory`
> - 永久方案：把中间人根证书导出为 `.pem` 并设 `NODE_EXTRA_CA_CERTS=/path/to/ca.pem`

## 本地开发

```bash
node dsh-add-timem-memory.mjs [选项]
```
